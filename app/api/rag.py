...
def _call_openai(prompt: str, model: str = "gpt-4o-mini") -> str:
    ...
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "당신은 문서 기반 질의응답 AI 어시스턴트입니다. 주어진 문서 컨텍스트를 바탕으로 정확하고 친절하게 답변하세요. 컨텍스트에 없는 내용은 추측하지 마세요."},
...
class RAGRequest(BaseModel):
    ...
    llm_provider: Optional[str] = Field(default=None, description="LLM 제공자 (openai, ollama, gemini)")
...
async def rag_chat(req: RAGRequest):
    ...
    for i, hit in enumerate(search_result, 1):
        ...
        # 컨텍스트용 전체 내용
        context_parts.append(f"[문서 {i}] {metadata.get('title', '문서')}, 페이지 {metadata.get('page_no', '?')}:\n{content}")
...
내용이 없을 시 참고 문서는 표시하지 마세요 """
    ...
    try:
        if llm_provider == "openai":
            ...
            raise HTTPException(status_code=400, detail=f"지원하지 않는 LLM 제공자: {llm_provider}")
            
    except Exception as e:
        logger.error(f"[RAG] LLM call failed: {e}")
        raise HTTPException(status_code=500, detail=f"LLM 호출 실패: {str(e)}")
    
    logger.info(f"[RAG] question='{req.question[:50]}...' sources={len(sources)} provider={llm_provider}")
