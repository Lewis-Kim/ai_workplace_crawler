...
class EmbeddingSettingsUpdate(BaseModel):
    model_key: str = Field(
        ..., description="임베딩 모델 키 (openai_large, nomic, bge_m3 등)"
    )
...
async def update_embedding_settings(req: EmbeddingSettingsUpdate):
    ...
    if not result["success"]:
        raise HTTPException(
            status_code=400, detail=result.get("error", "설정 변경 실패")
        )

    logger.info(
        f"[SETTINGS] Embedding changed: {result['old_model']} -> {result['new_model']}"
    )
...
async def reindex_embeddings(doc_id: Optional[int] = None):
    ...
    logger.info(
        f"[SETTINGS] Starting reindex with model_key={model_key}, doc_id={doc_id}"
    )

    try:
        if doc_id is None:
            result = await rebuild_vectors_async(model_key=model_key)
        else:
        ...
        return {
            ...
            "error": result.error,
            "message": f"재인덱싱 완료: {result.success_chunks}/{result.total_chunks} chunks"
            if result.success
            else f"재인덱싱 실패: {result.error}",
        }
    ...
    try:
        ...
        for col in collections.collections:
            try:
                ...
                vectors_config = info_any.config.params.vectors
                ...
                collection_list.append(
                    {
                        "name": col.name,
                        "vectors_count": getattr(info_any, "vectors_count", None),
                        "points_count": info_any.points_count,
        ...
        raise HTTPException(
            status_code=500, detail=f"Collection 목록 조회 실패: {str(e)}"
        )
...
async def update_collection(req: CollectionSettingsUpdate):
    ...
    try:
        ...
        logger.info(
            f"[SETTINGS] Collection changed: {result['old_collection']} -> {result['new_collection']}"
        )
...
async def verify_settings():
    ...
    try:
        ...
        provider_row = (
            ...
            .filter(SystemSettings.setting_key == "llm_provider")
            .first()
        )
        model_row = (
            db.query(SystemSettings)
            .filter(SystemSettings.setting_key == "llm_model")
            .first()
        )
        embed_row = (
            db.query(SystemSettings)
            .filter(SystemSettings.setting_key == "embedding_model_key")
            .first()
        )

        if provider_row:
            db_provider = cast(Optional[str], provider_row.setting_value)
            updated_at = cast(Optional[datetime], provider_row.updated_at)
            db_updated_at = str(updated_at) if updated_at is not None else None
        if model_row:
            db_model = cast(Optional[str], model_row.setting_value)
        if embed_row:
            db_embedding = cast(Optional[str], embed_row.setting_value)

        db_synced = (
            db_provider == runtime_provider
            and db_model == runtime_model
            and db_embedding == runtime_embedding
        )
        db.close()
    except Exception as e:
    ...
    # 전체 동기화 상태
    all_synced = (
        runtime_provider == db_provider == rag_provider
        and runtime_model == db_model == rag_model
        and (db_embedding is None or db_embedding == runtime_embedding)
    )
    ...
    return {
        ...
        "embedding": {
            ...
            "synced": runtime_embedding == env_embedding
            or db_embedding == runtime_embedding,
        },
        "all_synced": all_synced,
        "message": "All settings are synchronized"
        if all_synced
        else "Settings may be out of sync",
    }
